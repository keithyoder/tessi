class PlanoVerificarAtributosController < ApplicationController
  before_action :set_plano_verificar_atributo, only: [:show, :edit, :update, :destroy]
  ...
  def index
    ...
    respond_to do |format|
      format.html
      format.csv { send_data @plano_verificar_atributos.except(:limit, :offset).to_csv, filename: "plano_verificar_atributos-#{Date.today}.csv" }
    end
  end
  ...
  def create
    ...
    respond_to do |format|
      if @plano_verificar_atributo.save
        format.html { redirect_to @plano_verificar_atributo, notice: 'Plano verificar atributo was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @plano_verificar_atributo.update(plano_verificar_atributo_params)
        format.html { redirect_to @plano_verificar_atributo, notice: 'Plano verificar atributo was successfully updated.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to plano_verificar_atributos_url, notice: 'Plano verificar atributo was successfully destroyed.' }
